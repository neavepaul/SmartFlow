        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as pc_socket:
            ...
            while self._step < self._max_steps:
                ...
                try:
                    ...
                    req_send_time = timeit.default_timer()

                    # Serialize and send the array
                    serialized_data = pickle.dumps(current_state)
                    pc_socket.sendall(serialized_data)

                    # Receive result from Raspberry Pi
                    received_data = pc_socket.recv(1024)
                    action = pickle.loads(received_data)

                    latency = timeit.default_timer() - req_send_time
